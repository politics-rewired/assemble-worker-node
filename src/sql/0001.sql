...
create trigger _500_queue_pending_jobs after update on assemble_worker.job_queues  for each row when (OLD.created = false and NEW.created = true) execute procedure assemble_worker.tg_job_queues__after_queue_create();

-- Notify worker of new jobs
create function assemble_worker.tg_jobs__notify_new_jobs() returns trigger as $$
begin
  perform pg_notify('assemble-worker', NEW.queue_name || '|' || NEW.payload::text);
  return NEW;
end;
$$ language plpgsql;

create trigger _900_notify_worker after insert on assemble_worker.jobs for each row execute procedure assemble_worker.tg_jobs__notify_new_jobs();
...
begin
  ...
  if v_queue_exists then
    insert into assemble_worker.jobs (queue_name, payload, max_attempts)
